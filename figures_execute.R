...
# the result files contain the output of the HOLS check, the latest commit, the respective sample size
# and the random seeds for each simulation run in order to recreate individual results if desired
SEM_folder <- SEM_simulation()
# plot the results as in the paper
...
# run the simulation and store information on save location that was automatically created based on time
# the result files contain the output of the HOLS check (low-dimensional and high-dimensional),
# the latest commit
# and the random seeds for each simulation run in order to recreate individual results if desired
...
# run the simulation and store information on save location that was automatically created based on time
# the result files contain the output of the HOLS check (low-dimensional and high-dimensional),
# the latest commit, the respective sample size
# and the random seeds for each simulation run in order to recreate individual results if desired
